...
// Permisos: profesor ahora tiene payments pero sin ver totales financieros
const PERMISOS = {
  admin:    ["dashboard","students","payments","attendance","belts","finance","events","users"],
  profesor: ["attendance","students","payments"],
  alumno:   ["mi_asistencia","mis_pagos"],
};

// Auto-refresh cada 30 segundos para sincronizar cambios de otros dispositivos
...
const LoginScreen = ({ onLogin }) => {
  const [mode, setMode] = useState("login"); // login | forgot | change_pass
  ...
  const handleChangePass = async () => {
    ...
    } catch { setErr("Error al actualizar. Intenta de nuevo."); }
    setLoading(false);
  };
        ...
        <div className="bg-white/5 border border-white/10 rounded-3xl p-8 shadow-2xl">
          ...
          {mode === "login" && (
            <>
              ...
              <button onClick={()=>{setMode("forgot");setErr("");}} className="w-full mt-3 text-center text-sm text-slate-500 hover:text-amber-400 transition-colors">
          ...
          {mode === "forgot" && (
            <>
              ...
              <p className="text-slate-400 text-sm mb-6">Ingresa tu correo registrado para continuar.</p>
              {err && <div className="mb-4 p-3 rounded-xl bg-red-500/20 border border-red-500/30 text-red-400 text-sm">{err}</div>}
              <Field label="Correo registrado"><Input type="email" value={forgotEmail} onChange={(e)=>setForgotEmail(e.target.value)} placeholder="tu@correo.com" onKeyDown={(e)=>e.key==="Enter"&&handleForgotSearch()} /></Field>
              <button onClick={handleForgotSearch} disabled={loading} className="w-full mt-6 py-3.5 rounded-xl font-bold text-sm text-[#020617] disabled:opacity-60" style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)" }}>
                {loading ? "BUSCANDO..." : "CONTINUAR"}
              </button>
              <button onClick={()=>{setMode("login");setErr("");}} className="w-full mt-3 text-center text-sm text-slate-500 hover:text-amber-400 transition-colors">← Volver al inicio</button>
            </>
          )}
          ...
          {mode === "change_pass" && (
            <>
              ...
              <div className="space-y-4">
                ...
                <Field label="Confirmar contraseña"><Input type="password" value={confirmPass} onChange={(e)=>setConfirmPass(e.target.value)} placeholder="Repite la contraseña" onKeyDown={(e)=>e.key==="Enter"&&handleChangePass()} /></Field>
              </div>
              <button onClick={handleChangePass} disabled={loading} className="w-full mt-6 py-3.5 rounded-xl font-bold text-sm text-[#020617] disabled:opacity-60" style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)" }}>
...
// ─── CHANGE PASSWORD MODAL (para usuarios ya logueados) ───────────────────────
const ChangePasswordModal = ({ currentUser, onClose }) => {
  ...
  const save = async () => {
    setErr(""); setOk("");
    if (!form.oldPass || !form.newPass || !form.confirm) { setErr("Completa todos los campos"); return; }
    ...
    try {
      ...
      const me = users?.[0];
      if (!me || me.password !== form.oldPass) { setErr("La contraseña actual es incorrecta"); setSaving(false); return; }
      await db.update("users", currentUser.id, { password: form.newPass });
  ...
  return (
    <Modal title="Cambiar Contraseña" onClose={onClose}>
      <div className="space-y-4">
        ...
        <Field label="Nueva contraseña"><Input type="password" value={form.newPass} onChange={set("newPass")} placeholder="Mínimo 6 caracteres" /></Field>
      ...
      <div className="flex gap-3 mt-6">
        ...
        <button onClick={save} disabled={saving} className="flex-1 py-3 rounded-xl text-[#020617] text-sm font-bold disabled:opacity-60" style={{ background:"linear-gradient(135deg,#f59e0b,#d97706)" }}>{saving?"Guardando...":"Cambiar Contraseña"}</button>
      </div>
    </Modal>
  );
};
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white/3 border border-white/8 rounded-2xl p-6">
          <h3 className="text-lg font-bold text-white mb-4" style={{ fontFamily:"'Bebas Neue',sans-serif" }}>INGRESOS {today.getFullYear()}</h3>
...
const StudentsPage = ({ students, reload, canEdit }) => {
  ...
  const StudentForm = ({ student, onClose }) => {
    ...
    const save = async () => {
      ...
      setSaving(true);
      if (student) {
        await db.update("students", student.id, form);
      } else {
        // Crear alumno
        const newStudent = await db.insert("students", form);
        // Crear usuario automáticamente si tiene correo y contraseña
        if (form.correo && userPass) {
          await db.insert("users", { nombre:`${form.nombres} ${form.apellidos}`, email:form.correo, password:userPass, role:"alumno" });
        }
      }
      await reload();
      setSaving(false);
    ...
    return (
      <Modal title={student?"Editar Alumno":"Nuevo Alumno"} onClose={onClose} wide>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Correo" className="col-span-2"><Input value={form.correo||""} onChange={set("correo")} placeholder="correo@mail.com (se usará para el acceso)" /></Field>
          {!student && (
            <Field label="Contraseña de acceso del alumno/padre" className="col-span-2">
              <Input type="password" value={userPass} onChange={(e)=>setUserPass(e.target.value)} placeholder="Mínimo 6 caracteres (opcional)" />
              <p className="text-xs text-slate-500 mt-1">Si ingresas correo y contraseña, se creará automáticamente un usuario para el alumno/padre.</p>
            </Field>
          )}
...
const PaymentsPage = ({ students, pagos, reload, isAdmin }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Solo admin ve totales financieros */}
      {isAdmin && (
        <div className="grid grid-cols-3 gap-4">
...
const UsersPage = ({ currentUser, setCurrentUser, allUsers, reloadUsers }) => {
    ...
    return (
      <Modal title={user?"Editar Usuario":"Nuevo Usuario"} onClose={onClose}>
        <div className="space-y-4">
          ...
          <Field label="Rol">
            <div className="grid grid-cols-3 gap-2 mt-1">
              {[{id:"admin",label:"Admin",desc:"Acceso total",color:"#f59e0b"},{id:"profesor",label:"Profesor",desc:"Asistencia + Pagos + Alumnos",color:"#3b82f6"},{id:"alumno",label:"Alumno/Padre",desc:"Solo sus datos",color:"#22c55e"}].map((r) => (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[{role:"admin",color:"#f59e0b",label:"Administrador",perms:["Todo el sistema"]},{role:"profesor",color:"#3b82f6",label:"Profesor",perms:["Asistencia","Alumnos","Registrar pagos"]},{role:"alumno",color:"#22c55e",label:"Alumno/Padre",perms:["Ver su asistencia","Ver sus pagos"]}].map((r) => (
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Auto-refresh para sincronizar con otros dispositivos
    ...
    <div className="min-h-screen flex" style={{ background:"#020617", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 flex flex-col border-r border-white/8 transition-transform duration-300 ${sidebarOpen?"translate-x-0":"-translate-x-full lg:translate-x-0"}`} style={{ background:"#080f1f" }}>
        ...
        <div className="p-4 border-t border-white/8 space-y-2">
          {/* Botón cambiar contraseña disponible para TODOS los roles */}
